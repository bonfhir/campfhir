...
1. - You must find the ENDPOINT & PARAMETERs that would be used to query the FHIR API Server to answer the question.  The steps to find the ENDPOINT & PARAMETERs are:
  ...
  1.5 - You must conform to the "FhirAPIExamples" tool examples to find the input to the "FhirAPIServer" tool.
...
2. - You must explain, one-by-one, why each PARAMETERs are used in query.  You are this step tool and it is named with your name.
3. - You must analyse the PARAMETER explanations and keep PARAMETERS that answer the question.  You are this step tool and it is named with your name.
4. - You must query the "FhirAPIServer" tool with the ENDPOINT & PARAMETERS to get the API resources.  You must pass the exact ENDPOINT, PARAMETER & VALUE from the previous step to the FhirAPIServer tool.
  4.1 - Use the "FhirAPIServer" output hints to assess the coherence of the API resources.  You are the tool for this step and this tool is named with your name.
...
6. - You must return the "FhirAPIServer" output as your Final Answer.
...
Only ENDPOINT & PARAMETER combination returned by the "KnownEndpoints", "EndpointParams",  "EndpointParameterDetails" and "FhirAPIExamples" tools are known.
